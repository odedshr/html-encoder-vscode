...
abstract class JSNodeAbstract {
	...
	private getDOMParser(domParserInstance?: DOMParser): DOMParser {
		...
		const _get = (item: KeydObject, key: string) => item[key];
	...
	_getSetProxy(map: KeydObject) {
		return new Proxy(map, {
			...
			set: (map: KeydObject, prop: string, value: any) => {
				const property = map[prop];
	...
	_getValue(data: KeydObject, path: string): any {
		...
		return path[0] === '!'
			...
			: path
					.split('.')
					.reduce((ptr: KeydObject, step: string) => (ptr && ptr.hasOwnProperty(step) ? ptr[step] : undefined), data);
	}
