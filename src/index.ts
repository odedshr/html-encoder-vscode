...
class NodeParser {
		...
		if (tagName.indexOf('?') === 0) {
			...
			return [
				this._getAppendLivableString(
					`self._getHTMLNode(self._getValue(self.data, '${tagName.substring(2)}'))`,
					node.nodeValue,
					'html'
				),
			];
			...
			return [
				this._getAppendLivableString(
					`docElm.createTextNode(self._getValue(self.data, '${tagName.substring(1)}'))`,
					node.nodeValue,
					'text'
				),
			];
	...
	_getAppendLivableString(nodeString: string, nodeValue: string, type: string): string {
		const addToSetString =
			nodeValue.indexOf('#') === 0 ? `self.set['${nodeValue.substring(1)}'] = { node, type: '${type}' };` : '';
