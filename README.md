...
1. Add 'html-encode' section in your `package.json`:
  ...
  html-encode-to-ts?: boolean
   ...
   `source` can use * as wildcard, for example `src/**/*.template.html`.
   Unless specifying target file the output will be the same filename as the source but with js/ts extension.
   When `html-encode-to-ts` is set to true output file will be typescript.
2. Run 'node ./node_modules/html-encoder/dist/cli.js`
